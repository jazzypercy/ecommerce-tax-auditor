...
import { Shield, ShoppingBag, Box, Download, AlertTriangle, ClipboardList, FileSpreadsheet, Info, RefreshCw } from 'lucide-react';
...
import BookkeepingGuide from './components/BookkeepingGuide';
...
export default function App() {
  ...
  const accent = selectedPlatform === 'lazada'
  ...
  const extractFile = (input) => {
    ...
    if (input?.target?.files) return input.target.files[0];
    return input;
  };
  ...
  if (currentView === 'welcome') {
    return <WelcomePage onSelectPlatform={handleSelectPlatform} />;
  }
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-800 font-sans antialiased">
      ...
      <BookkeepingGuide isOpen={isGuideOpen} onClose={() => setIsGuideOpen(false)} />

      <nav className="bg-white border-b border-slate-200 px-6 py-4 flex flex-wrap items-center justify-between gap-3 sticky top-0 z-40 shadow-sm">
        <div className="flex items-center gap-3">
          <Shield className={`w-6 h-6 ${accent.text}`} />
          <span className="font-bold text-xl tracking-tight text-slate-900">Profit Guard PH</span>
          <span className={`hidden sm:inline-block text-xs font-semibold px-2.5 py-1 rounded-full ${accent.bg} ${accent.text}`}>
            {selectedPlatform === 'lazada' ? 'Lazada Mode' : 'Shopee Mode'}
          </span>
        </div>
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            className="flex items-center gap-1.5 text-xs font-medium text-slate-400 hover:text-slate-600 px-3 py-2 rounded-lg hover:bg-slate-50 transition-colors"
          >
      ...
      <main className="max-w-6xl mx-auto p-6 md:p-8">
          ...
          {selectedPlatform === 'shopee' && (
            <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm">
              <h2 className="text-lg font-bold text-orange-600 mb-4 flex items-center gap-2">
                <ShoppingBag className="w-5 h-5" /> Shopee CSV Statement
              </h2>
          ...
          {selectedPlatform === 'lazada' && (
            <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm">
              <h2 className="text-lg font-bold text-blue-600 mb-4 flex items-center gap-2">
                <Box className="w-5 h-5" /> Lazada CSV Statement
              </h2>
          ...
          <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm">
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-lg font-bold text-purple-700 flex items-center gap-2">
                <ClipboardList className="w-5 h-5" /> Warehouse Return Log
              </h2>
              <button
        ...
        {/* Empty state */}
        {((selectedPlatform === 'shopee' && !shopeeData) || (selectedPlatform === 'lazada' && !lazadaData)) && (
          <div className="bg-white border border-dashed border-slate-300 rounded-xl p-10 text-center text-slate-400 mb-8">
            <p className="text-sm">
              Upload your {selectedPlatform === 'shopee' ? 'Shopee' : 'Lazada'} statement above to generate your audit report.
            </p>
          </div>
        )}

        {/* LEAKAGE MODULE 1: Lost Return Tracker */}
        {lostReturns.length > 0 && (
          <div className="bg-white rounded-xl border-l-4 border-l-purple-600 border border-slate-200 shadow-sm p-6 mb-8">
        ...
        {/* LEAKAGE MODULE 2: Shipping Overcharges */}
        {shippingDiscrepancies.length > 0 && (
          <div className="bg-white rounded-xl border-l-4 border-l-orange-500 border border-slate-200 shadow-sm p-6 mb-8">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-4">
              <div>
        ...
        {/* LEAKAGE MODULE 3: RTS Failed-Delivery Leakage */}
        {rtsLeakage.length > 0 && (
        ...
        {/* LEAKAGE MODULE 4: Commission Creep */}
        {commissionCreep.length > 0 && (
          <div className="bg-white rounded-xl border-l-4 border-l-amber-500 border border-slate-200 shadow-sm p-6 mb-8">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-4">
              <div>
                ...
                <p className="text-sm text-slate-500">Orders where total platform fees exceed a 10.5% baseline threshold.</p>
              </div>
              <button
                  ...
                  {reconciliationResults.map((row) => (
                    <tr key={`${row.orderId}-${row.discrepancyType}`} className="border-b hover:bg-rose-50 transition-colors">
                      ...
                      <td className="p-2 text-xs">
                        <span className={`px-2 py-0.5 rounded text-xs font-semibold ${row.platform === 'Shopee' ? 'bg-orange-50 text-orange-700' : 'bg-blue-50 text-blue-700'}`}>
                          {row.platform ?? '—'}
                        </span>
                      </td>
                      ...
                      <td className="p-2">
                        <span className={`px-2 py-0.5 rounded text-xs font-bold ${row.discrepancyType === 'LOST_INVENTORY' ? 'bg-rose-100 text-rose-700' : 'bg-amber-100 text-amber-700'}`}>
